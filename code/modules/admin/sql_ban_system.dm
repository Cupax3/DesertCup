...
/datum/admins/proc/ban_panel(player_key, player_ip, player_cid, role, duration = 1440, applies_to_admins, reason, edit_id, page, admin_key)
	...
	if(edit_id)
		...
		var/list/command_lists = list("BoS Command" = GLOB.brotherhood_command_positions,
							"Oasis Command" = GLOB.den_command_positions,
							"Legion Command" = GLOB.legion_command_positions,
							"NCR Command" = GLOB.ncr_command_positions,
							"Vault Command" = GLOB.vault_command_positions,
							"Followers Command" = GLOB.followers_command_positions
							)
		for(var/job in command_lists)
		...
		var/list/job_lists = list("Security" = GLOB.security_positions,
							"BoS" = GLOB.brotherhood_positions,
							"Oasis" = GLOB.den_positions,
							"Legion" = GLOB.legion_positions,
							"NCR" = GLOB.ncr_positions,
							"Vault" = GLOB.vault_positions,
							"Silicons" = GLOB.silicon_positions,
							"Tribal" = GLOB.tribal_positions,
							"Followers" = GLOB.followers_positions
							)
		...
		var/list/headless_job_lists = list("Silicon" = GLOB.silicon_positions,
		...
		for(var/department in headless_job_lists)
			...
			for(var/job in headless_job_lists[department])
				...
				break_counter++
			output += "</div></div>"
		var/list/long_job_lists = list("Ghost and Other Roles" = list(ROLE_BRAINWASHED, ROLE_DEATHSQUAD, ROLE_DRONE, ROLE_LAVALAND, ROLE_MIND_TRANSFER, ROLE_POSIBRAIN, ROLE_SENTIENCE),
